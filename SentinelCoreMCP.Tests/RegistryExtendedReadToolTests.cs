...
public sealed class RegistryExtendedReadToolTests
{
    ...
    public async Task ComListClasses_RespectsMaxRecords()
    {
        ...
        Assert.True(result.Success, $"Expected Success=true but got failure: {result.ErrorDetails}");
        List<RegistryExtendedReadTool.ComClassRecord> records = Assert.IsType<List<RegistryExtendedReadTool.ComClassRecord>>(result.Results);
        Assert.True(records.Count <= maxRecords, $"Expected at most {maxRecords} COM classes but got {records.Count}");
    }
