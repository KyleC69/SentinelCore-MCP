...
public sealed class NetworkExtendedReadToolTests
{
    ...
    #region Network_List_Listening_Ports tests
    ...
    public async Task NetworkListListeningPorts_RespectsMaxRecords()
    {
        ...
        Assert.True(result.Success, $"Expected Success=true but got failure: {result.ErrorDetails}");
        List<object> ports = Assert.IsType<List<object>>(result.Results);
        Assert.True(ports.Count <= maxRecords,
            $"Expected at most {maxRecords} ports but got {ports.Count}");
    }
    ...
    #region Network_Read_DNS_Cache tests
    ...
    #region Network_Read_ARP_Table tests
    ...
    #region Network_Read_Routing_Table tests
    ...
    #region Network_List_Shares tests
