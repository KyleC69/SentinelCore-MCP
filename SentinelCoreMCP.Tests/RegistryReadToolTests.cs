...
public sealed class RegistryReadToolTests
{
    ...
    #region GetHiveRoot tests (via reflection — internal method on internal RegistryHelper)

    private static readonly Type RegistryHelperType =
        typeof(RegistryReadTool).Assembly.GetType("SentinelCoreMCP.Tools.RegistryHelper")
        ?? throw new InvalidOperationException("RegistryHelper type not found.");

    private static object? InvokeGetHiveRoot(string hive)
    {
        MethodInfo? method = RegistryHelperType.GetMethod(
            "GetHiveRoot",
    ...
    #region FormatRegistryValue tests (via reflection — internal method on internal RegistryHelper)
    ...
    {
        string? result = InvokeFormatRegistryValue(new string[] { "a", "b", "c" }, (int)Microsoft.Win32.RegistryValueKind.MultiString);
    ...
    #region Registry_List_Key tests
    ...
    public async Task RegistryListKey_ValidKey_ReturnsTypedListingRecord()
    {
        ...
        RegistryReadTool.RegistryKeyListingRecord listing =
            Assert.IsType<RegistryReadTool.RegistryKeyListingRecord>(result.Results);
    ...
    public async Task RegistryListKey_RespectsMaxRecords()
    {
        ...
        RegistryReadTool.RegistryKeyListingRecord listing =
            Assert.IsType<RegistryReadTool.RegistryKeyListingRecord>(result.Results);
        Assert.True(listing.SubKeys.Count <= maxRecords,
            $"Expected at most {maxRecords} subkeys but got {listing.SubKeys.Count}");
    }
    ...
    #region Registry_Read_Value tests
    ...
    public async Task RegistryReadValue_ValidKeyValue_ReturnsTypedRecord()
    {
        ...
        Assert.True(result.Success);
        RegistryReadTool.RegistryValueReadRecord record =
            Assert.IsType<RegistryReadTool.RegistryValueReadRecord>(result.Results);
