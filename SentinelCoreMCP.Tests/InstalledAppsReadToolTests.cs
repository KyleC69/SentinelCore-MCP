...
public sealed class InstalledAppsReadToolTests
{
    ...
    #region Installed_Apps_List tests
    ...
    public async Task InstalledAppsList_ReturnsNonEmptyDictionaryList()
    {
        ...
        List<Dictionary<string, string?>> apps =
    ...
    public async Task InstalledAppsList_WithFilter_ReturnsMatchingApps()
    {
        ...
        Assert.All(apps, a =>
            Assert.Contains("Microsoft", a["DisplayName"], StringComparison.OrdinalIgnoreCase));
    }
    ...
    public async Task InstalledAppsList_RespectsMaxRecords()
    {
        ...
        List<Dictionary<string, string?>> apps =
            Assert.IsType<List<Dictionary<string, string?>>>(result.Results);
        Assert.True(apps.Count <= maxRecords,
            $"Expected at most {maxRecords} apps but got {apps.Count}");
    }
    ...
    #region Installed_Apps_MSI_List tests
    ...
    public async Task InstalledAppsMsiList_WhenSuccessful_ReturnsListContent()
    {
        ...
        if (!result.Success) { return; } // Skip if MSI enumeration unavailable
