...
internal static class SysinternalsHelper
{
    ...
    public sealed record SysinternalsBinaryInfo(
        string ToolName,
        string BinaryName,
        bool Available,
        string? Path,
    ...
    internal static ToolResult ProbeAvailability(string toolName)
    {
        ...
        if (path is null)
        {
            return ToolResult.Ok(
                new SysinternalsBinaryInfo(toolName, binaryName, Available: false, Path: null, FileVersion: null, FileDescription: null),
                $"Sysinternals {toolName} is not installed on this system.");
        }

        FileVersionInfo versionInfo = FileVersionInfo.GetVersionInfo(path);
        return ToolResult.Ok(
            new SysinternalsBinaryInfo(
                ToolName: toolName,
                BinaryName: Path.GetFileName(path),
                Available: true,
                Path: path,
                FileVersion: versionInfo.FileVersion,
                FileDescription: versionInfo.FileDescription),
            $"Sysinternals {toolName} is available.");
    }
    ...
    internal static ToolResult? ValidateArgument(string? value, string paramName)
    {
        ...
        if (value!.IndexOfAny(['"', '\'', '`', '&', '|', ';', '<', '>', '%', '$', '!', '\r', '\n']) >= 0)
        {
            return ToolResult.Fail(
                $"{paramName} contains characters that are not permitted: {value}. Arguments must not contain quotes or shell metacharacters.",
                "Sysinternals argument validation");
        }
    ...
    internal static async Task<ToolResult> RunAsync(string toolName, string arguments, string operation, int timeoutSeconds = 60)
    {
        ...
        {
            return ToolResult.Fail(
                $"Sysinternals tool '{toolName}' was not found. Install the Sysinternals suite and ensure {toolName}64.exe is on the PATH or in a well-known install directory.",
                operation);
        }
