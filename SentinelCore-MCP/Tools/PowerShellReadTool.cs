...
public sealed class PowerShellReadTool
{
    ...
    public static string? ValidateCommand(string command)
    {
        ...
        foreach (string pattern in ForbiddenPatterns)
        {
            if (command.Contains(pattern, StringComparison.OrdinalIgnoreCase))
            {
                return $"Command contains forbidden pattern: '{pattern}'. " +
                       "Pipeline operators, redirects, script blocks, and .NET type access are not allowed.";
            }
        }
        ...
        foreach (string token in tokens)
        {
            ...
            {
                return $"Command '{token}' is not in the allowed list. " +
                       "Only pre-approved read-only commands are permitted.";
            }
        }
    ...
    [Description("Executes a pre-approved, read-only PowerShell query in a sandboxed runspace. " +
                  "Only whitelisted commands are permitted. Pipeline operators, redirects, " +
                  "script blocks, and .NET type access are forbidden.")]
    public async Task<ToolResult> powershellQueryAsync(
        [Description("The PowerShell command to execute. Must be a single read-only command " +
                      "from the approved list. No pipelines, redirects, or script blocks.")] string command,
        [Description("Maximum number of result objects to return. Defaults to 50.")] int maxResults = DefaultMaxResults)
    {
